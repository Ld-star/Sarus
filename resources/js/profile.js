...
$(document).ready(function() {
    ...
    $("#token-life").val(getSession('defaultMin'));

    var sessionTimeFunc = () => {
        curTime = getCookie("remaining");
       
        sessionTime = setInterval(() => {
            if(getSession('IsChangeInfo') == 1) {
                setSession(0, 'IsChangeInfo');
                ReadUserSettings();
            }
            if (getCookie("remaining") >= 1000 ) {
                document.getElementById("renewToken").checked = getCookie('remember-renewToken') == 'true' ? true : false;
                $('#session-token').text(getSession());
                $("#remaining-token").text(get_time(getCookie("remaining") - 1000 ));
                $('#loginPopup').jqxWindow('close');
                
            } else{
                if(  getCookie('remember-renewToken') == 'false' ) {
                    ...
                    setCookie('remember-renewToken',userSettings.autoLogin ? 'true' : 'false');
                }
            }
    ...
    $('#renewToken').click(function() {
        settings.AutoLogin = $(this).is(':checked') ? true : false;
        setCookie("remember-renewToken", $(this).is(':checked') ? 'true' : 'false');
    });
    $('#saveSettings').click(function() {
        if (getSession() == undefined || getSession() == "") {
            ...
            userSettings.autoLogin = settings.AutoLogin;
            setSession(1, 'IsChangeInfo');
    ...
    $("#saveDefault").on('click', function() {
        if (getSession() == undefined || getSession() == "") {
            ...
            call_api_ajax("WriteUserJSONSettings", "post", JSON.stringify({
                ...
                setSession( settings.Token.DefaultMins , 'defaultMin');
                dialogWindow("Settings saved successfully", "information");
            });
